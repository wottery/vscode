...
export class SettingsEditor2 extends EditorPane {
	...
	private static readonly SUGGESTIONS: string[] = [
		`@${MODIFIED_SETTING_TAG}`, '@tag:usesOnlineServices', '@tag:sync', `@${EXTENSION_SETTING_TAG}`, `@${FEATURE_SETTING_TAG}`, `@${ID_SETTING_TAG}`
	];
	...
	private triggerSearch(query: string): Promise<void> {
		...
		if (query && query !== '@') {
			...
			if (this.hasSome(this.viewState.tagFilters) || this.hasSome(this.viewState.extensionFilters) || this.hasSome(this.viewState.featureFilters) || this.hasSome(this.viewState.idFilters)) {
