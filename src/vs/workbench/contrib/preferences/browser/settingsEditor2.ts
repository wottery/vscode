...
export class SettingsEditor2 extends EditorPane {
	...
	private triggerSearch(query: string): Promise<void> {
		...
		this.viewState.featureFilter = new Set<string>();
		if (query) {
			const parsedQuery = parseQuery(query);
			query = parsedQuery.query;
			parsedQuery.tags.forEach(tag => this.viewState.tagFilters!.add(tag));
			parsedQuery.featureFilters!.forEach(featureConfig => this.viewState.featureFilter!.add(featureConfig));
		...
		if (query && query !== '@') {
			...
			if (this.hasSome(this.viewState.tagFilters) || this.hasSome(this.viewState.extensionFilters) || this.hasSome(this.viewState.featureFilter)) {
